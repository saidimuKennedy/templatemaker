/**
 * Ad-hoc smoke checks for Plan 04 Inspector / Property Engine.
 * Run: npx tsx builder/inspector/smoke.ts
 */
...
assert(
  validateFieldValue(selectField, "sm") === undefined,
  "validateFieldValue accepts in-range select value",
);
assert(
...
const updatedNode = result.ok
  ? result.result.document.pages[0].root.children[0]
  : undefined;
